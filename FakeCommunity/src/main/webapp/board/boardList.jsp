...
<%
	...
	if(cusbodDAO.count(cri, "F") == (cri.getPageNum()-1) * cri.getAmount() && cusbodDAO.count(cri, "F") != 0){
		cri.setPageNum((cri.getPageNum()-1));
	}
	
	//board 테이블에서 전체글 리스트로 가져오기
	Vector<CusbodVO> boardList = cusbodDAO.selectPageOption(cri, "F");
	
	//전체 글개수 가져오기
	int totalCount = cusbodDAO.count(cri, "F");
						...
						<div class="panel-body">
							<div class="row" align="right">
              					<%
              						if(id != null){
              					%>
		                				<a  class="btn btn-primary" href="/board/boardWrite.jsp?PageNum=<%=pageDTO.getCri().getPageNum()%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>">
							...
							<table class="table table-bordered">
                				...
                				%>
                  						<tr class="datatr" onclick="location.href='/board/boardContent.jsp?num=<%=board.getCsnum()%>&cusid=<%=board.getCusid() %>&PageNum=<%=pageDTO.getCri().getPageNum()%>&type=<%=pageDTO.getCri().getType() %>&keyword=<%=pageDTO.getCri().getKeyword() %>'">
							...
							<ul class="pagination">
                				...
                				%>
		                				<li class="<%=(pageDTO.getCri().getPageNum() == i) ? "active":""%>"><a href="/board/boardList.jsp?PageNum=<%=i %>&type=<%=pageDTO.getCri().getType() %>&keyword=<%=pageDTO.getCri().getKeyword() %>#board"><%=i %></a></li>
                				<%
                					}
                				%>
                				<%
              						...
              						if(pageDTO.isNext()){
              					%>
		                				<li><a href="/board/boardList.jsp?PageNum=<%=pageDTO.getEndPage() + 1 %>&type=<%=pageDTO.getCri().getType() %>&keyword=<%=pageDTO.getCri().getKeyword() %>#board"><i class="material-icons">chevron_right</i></a></li>
		                		<%
              						} else {
                				%>
                						<li class="disabled">
